...
function scenario(){return {
  turns:24, buildRate:{1:1,2:1,3:0}, startPop:5,
  start:[["solar",0],["solar",0],["habitat",0],["iceExtractor",0],["waterPlant",0],["greenhouse",0]],
  /* generated by search-rs `gen` (required = greedy can do it; optionals = optimal-only headroom).
     score = directives completed: greedy baseline 5/8, search optimum 8/8. */
  directives:[
    ...
    {id:"D3",name:"Electronics",good:"electronics",rate:4,dur:2,deadline:13,req:["D2"],must:true,reward:{unlock:["assembler","lab"],buildRate:{3:1}},rp:120},
    {id:"D4",name:"Assembly",good:"components",rate:3,dur:3,deadline:18,req:["D3"],must:true,reward:{},rp:160},
    {id:"D5",name:"Datacore",good:"research",rate:3,dur:2,deadline:23,req:["D4"],must:true,reward:{},rp:260},
    {id:"D6",name:"Glassworks",good:"glass",rate:7,dur:2,deadline:16,req:[],must:false,reward:{},rp:50},
    {id:"D7",name:"Foodbelt",good:"food",rate:12,dur:2,deadline:12,req:[],must:false,reward:{},rp:50},
    {id:"D8",name:"Circuits",good:"electronics",rate:8,dur:2,deadline:9,req:[],must:false,reward:{},rp:50}
  ]};}
...
function applyReward(S,d){var r=d.reward||{};if(r.unlock)for(var i=0;i<r.unlock.length;i++)S.unlocked[r.unlock[i]]=1;
  if(r.buildRate)for(var bt in r.buildRate)S.buildRate[bt]=get(S.buildRate,bt)+r.buildRate[bt];if(r.immig)S.immig+=r.immig;}
